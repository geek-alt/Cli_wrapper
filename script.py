...
try:
    ...
    HAS_PYGMENTS = False
    # Create dummy variables for when pygments is not available
    highlight = None  # type: ignore
    guess_lexer_for_filename = None  # type: ignore
    TextLexer = None  # type: ignore
    TerminalFormatter = None  # type: ignore
...
if HAS_READLINE:
    try:
        ...
        pass
    atexit.register(lambda: readline.write_history_file(HISTORY_FILE))  # type: ignore
...
def highlight_code(filename: str, code: str) -> str:
    if not HAS_PYGMENTS:
        return code
    try:
        lexer = guess_lexer_for_filename(filename, code)  # type: ignore
    except Exception:
        lexer = TextLexer()  # type: ignore
    return highlight(code, lexer, TerminalFormatter())  # type: ignore
...
def setup_completion():
    ...
    def completer(text, state):
        try:
            buf = readline.get_line_buffer()  # type: ignore
